    ...
    <div className="min-h-screen bg-slate-50">
      <aside className="fixed inset-y-0 left-0 hidden w-60 border-r border-slate-200 bg-white p-4 md:block">
        <div className="mb-6 flex items-center gap-2 px-2">
          ...
          <span className="text-lg font-bold text-slate-800">Portask</span>
        </div>
        <nav className="space-y-1">
          ...
          {user.role === "ADMIN" ? (
            <>
              <div className="pt-3 text-xs font-semibold uppercase tracking-wide text-slate-400">Admin</div>
              {adminItems.map((item) => (
                <NavLink key={item.to} to={item.to} end={item.end} className={linkBase}>
                  {item.label}
                </NavLink>
      ...
      <div className="md:pl-60">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur">
          <div className="md:hidden font-bold text-slate-800">Portask</div>
