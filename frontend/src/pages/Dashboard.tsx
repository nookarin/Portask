  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Everything that needs your attention, at a glance.</p>
      </div>
      ...
      {role === "EMPLOYEE" ? (
        <section>
                ...
                {data.myTasks.map((t) => (
                  <li key={t.id} className="flex items-center justify-between gap-4 py-3">
                    <div className="min-w-0">
                      <Link to={`/projects/${t.project.id}`} className="font-medium text-slate-800 hover:text-brand-600">
                        {t.title}
                      </Link>
                      <div className="text-xs text-slate-500">{t.project.name}</div>
                    </div>
                    <div className="flex shrink-0 items-center gap-2">
                      {t.dueDate ? <span className="text-xs text-slate-400">{formatDateTime(t.dueDate)}</span> : null}
          ...
          {data.tasksDueSoon.length > 0 ? (
            <p className="mt-2 text-sm text-amber-600">{data.tasksDueSoon.length} task(s) due within 7 days.</p>
          ) : null}
        </section>
      ) : null}
      ...
      {role === "CLIENT" ? (
        <section>
          <h2 className="mb-3 text-lg font-semibold text-slate-800">Latest updates</h2>
          <div className="space-y-3">
            {data.recentUpdates.length === 0 ? <EmptyState title="No updates yet" /> : null}
            {data.recentUpdates.map((u) => (
              <Card key={u.id}>
                <Link to={`/projects/${u.project.id}`} className="font-semibold text-slate-800 hover:text-brand-600">
                  {u.title}
                </Link>
                <div className="mt-1 text-sm text-slate-500">{u.body}</div>
                <div className="mt-2 flex items-center gap-2">
                  <ProgressBar value={u.progress} />
                  <span className="text-xs font-medium text-slate-500">{u.progress}%</span>
                </div>
          ...
          {data.upcomingMilestones.length > 0 ? (
            <div className="mt-6">
              <h2 className="mb-3 text-lg font-semibold text-slate-800">Upcoming milestones</h2>
              <Card>
                <ul className="divide-y divide-slate-100">
                  {data.upcomingMilestones.map((m) => (
                    <li key={m.id} className="flex items-center justify-between py-2 text-sm">
                      <span className="text-slate-800">{m.name}</span>
                      <span className="text-slate-500">{m.project.name} · {formatDateTime(m.dueDate)}</span>
                    </li>
                  ))}
                </ul>
              </Card>
              ...
              {data.recentActivity.map((a) => (
                <li key={a.id} className="flex items-start justify-between gap-4 py-3 text-sm">
                  ...
                  <div className="shrink-0 text-right text-xs text-slate-400">
                    ...
                    {"project" in a && a.project ? <div className="text-brand-600">{a.project?.name}</div> : null}
                  </div>
                </li>
              ))}
            </ul>
